...
def create_gmail_users_table():
    """Create a table to store Gmail user credentials if it does not exist."""
    conn = sqlite3.connect("gmail_info.db")
    cursor = conn.cursor()
...
def fetch_emails_for_user(email_to_fetch):
    ...
    try:
        ...
        if emails:
            ...
            print("Unique emails successfully stored in the database.")
        else:
